//#include "../../mmu.h"
//#include "memlayout.h"
#include "/home/ylq/win_share/my_kernel/raspi_my_os/enable_mmu/kernel/arch/aarch64/mmu.h"
#include "/home/ylq/win_share/my_kernel/raspi_my_os/enable_mmu/kernel/arch/aarch64/board/raspi3/memlayout.h"

/*
    在Linux中,4级页表的名称为 PGD -> PUD -> PMD -> PTE
    我们沿用这个命名,4级页表中,每个表项的粒度依次为 512G 1G 2M 4K 
    因为在树莓派 我们可用的物理内存范围是0-0x3F000000 共1008M 而在这之上,需要定义内存类型为Device
    所以,为了实现合适的恒等映射,我们需要至少使用3级页表的粒度,才能合适地配置内存类型
*/

// 在此粒度下,每个项目代表2M的内存 前204个是普通内存 后8个为设备内存
...
// 此粒度下 每个项目代表1G内存 在0-1G内存指向kpmd即可 1G-2G设置为Device内存
__attribute__((__aligned__(PGSIZE)))
uint64_t kpud[512]={
